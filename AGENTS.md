...
MVP implementation in progress. The repo now contains a Next.js App Router application, Supabase migrations, server-side pipeline modules, fraud patterns, scripts, and tests. Do not assume a module exists without checking, but also do not treat the repo as documentation-only.
...
- `npm run test:e2e` — Playwright happy-path flow.
- `npm run patterns:validate` — validate the fraud pattern library.
